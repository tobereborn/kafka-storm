...
public class MiniZooKeeperCluster {
	...
	private int selectClientPort(int seedPort) {
		...
		if (returnClientPort == 0) {
			...
			// client port.
			// If the default client port is not specified, starting with a
			// random port.
			// The random port is selected from the range between 49152 to
			// 65535. These ports cannot be
	 ...
	 * @return ClientPort server bound to, -1 if there was a binding problem and
	 *         we couldn't pick another port.
	 * @throws IOException
	...
	public int startup(File baseDir, int numZooKeeperServers) throws IOException, InterruptedException {
		...
		for (int i = 0; i < numZooKeeperServers; i++) {
			...
			if (hasValidClientPortInList(i)) {
				...
				tentativePort = selectClientPort(tentativePort); // update the
																	// seed
			...
			// We have selected a port as a client port. Update clientPortList
			// if necessary.
			if (clientPortList.size() <= i) { // it is not in the list, add the
												// port
				clientPortList.add(currentClientPort);
			} else if (clientPortList.get(i) <= 0) { // the list has invalid
														// port, update with
														// valid port
	...
	public void shutdown() throws IOException {
		...
		for (ZooKeeperServer zkServer : zooKeeperServers) {
			// explicitly close ZKDatabase since ZookeeperServer does not close
			// them
	...
	/**
	 * @return clientPort return clientPort if there is another ZK backup can
	 *         run when killing the current active; return -1, if there is no
	 *         backups.
	 * @throws IOException
	 * @throws InterruptedException
	 */
